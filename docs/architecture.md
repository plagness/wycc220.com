...
Модульный `@Legion220Bot` для трёх пространств: канал `@Wycc220fun`, чат `@WyccChat` и личные диалоги. Telegram transport отделён от модулей, а Twitch live monitor публикует domain event через channel publisher. Production использует webhook с отдельным secret.
...
Дизайн-токены и переиспользуемые React-примитивы без бизнес-логики и сетевых запросов. Это общий визуальный контракт сайта; композиция конкретных страниц остаётся в `apps/web`.

### `packages/game.sdk`

Минимальный версионируемый протокол между изолированной игрой и host-платформой. Сообщения не дают доступа к cookies, секретам или произвольному выполнению команд.

### `games/starter.babylon`

Собираемый reference-проект 3D-игры на Babylon.js. Он задаёт структуру, `game.manifest.json`, безопасную интеграцию с host и базовые browser/Telegram требования, но не ограничивает каталог одним движком.
